...
if "%1" == "run" (
    ...
    java -cp "!CLASSPATH!" com.iot.plc.MainApp
    pause
    exit /b
)
...
if not exist "lib\netty-all-4.1.94.Final.jar" (
    echo 下载Netty库...
    powershell -Command "Invoke-WebRequest -Uri 'https://repo1.maven.org/maven2/io/netty/netty-all/4.1.94.Final/netty-all-4.1.94.Final.jar' -OutFile 'lib\netty-all-4.1.94.Final.jar'"
)
...
REM 编译Java文件
javac -encoding UTF-8 -cp "%CLASSPATH%" -d target\classes src\main\java\com\iot\plc\*.java src\main\java\com\iot\plc\ui\*.java src\main\java\com\iot\plc\model\*.java src\main\java\com\iot\plc\database\*.java src\main\java\com\iot\plc\scheduler\*.java src\main\java\com\iot\plc\service\*.java src\main\java\com\iot\plc\util\*.java

if %errorlevel% equ 0 (
    echo 编译成功！
    echo 运行程序请执行: java -cp "%CLASSPATH%;target\classes" com.iot.plc.MainApp
) else (
    echo 编译失败！
)
